...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.position += moveDirection.normalized * moveSpeed * ((shift != 0) ? 2 : 1) * Time.deltaTime;

        bool isMoving = hor != 0 || vert != 0;
        if (isMoving && shift != 0) {
            ChangeAnimation(AnimationNames.RUN, 0.05f);
            return;
        }
        ChangeAnimation(isMoving ? AnimationNames.CROUCH_WALK : AnimationNames.CROUCH_IDLE, isMoving ? 0.05f : 0.15f);
    }
    ...
    private void ChangeAnimation(string animationName, float fadeTime)
    {
        if (!animator.GetCurrentAnimatorStateInfo(0).IsName(animationName) && !animator.IsInTransition(0)) {
            animator.CrossFade(animationName, fadeTime, 0);
        }
    }
